...
void CPlugin::shut()
{
  ...
  m_hWnd = NULL;
	...
	{
		ScriptableObject *pObj = (ScriptableObject *) m_pScriptableObject;
		pObj->Detatch ();
...
int16 CPlugin::handleEvent(void* event)
{
	...
	ScriptableObject *pObj = (ScriptableObject *) GetScriptableObject();
...
CPlugin::GetScriptableObject()
{
  if (m_pScriptableObject == NULL)
	{
		m_pScriptableObject = NPN_CreateObject (m_pNPInstance, &ScriptableObject::_npclass);
